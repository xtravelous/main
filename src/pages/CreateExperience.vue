<template>
  <div>
                  ...
                  <div v-show="currentTab === 'Cover'">
                    <v-container fluid>
                      <v-layout row wrap>
                        ...
                        <v-flex xs12>
                          <v-text-field clearable v-model="experienceData.name" placeholder="Enter experience name"></v-text-field>
                        </v-flex>
                        <v-flex xs12 sm12 md12 lg12 xl12>
                        ...
                        <v-flex xs12 sm12 md12 lg12 xl12 class="pt-2">
                          ...
                          <div v-show="hasCoverPhoto" class="pt-4">
                            ...
                            <v-flex xs12 sm12 md12 lg12 xl12 class="pt-2">
                              ...
                              <v-btn color="orange darken-1" dark @click="openThumbnailUploader">
                                <v-icon left>camera_alt</v-icon> Upload thumbnail
                              </v-btn>
                              </v-flex>
                            </div>
                          </v-layout>
                        ...
                        <v-container fluid>
                          <v-layout row wrap>
                            ...
                            <v-flex xs12 sm12 md12 lg12 xl12 class="pt-4">
                              ...
                              <v-flex xs12 sm12 md9 lg9 xl9>
                                <v-text-field v-model="experienceData.location.streetAddress" placeholder="House name/number + street/road"></v-text-field>
                              </v-flex>
                              <v-flex xs12 sm12 md12 lg12 xl12>
                                <div class="title">Apt, Suit, Bldg. (optional)</div>
                              </v-flex>
                              <v-flex xs12 sm12 md9 lg9 xl9>
                                <v-text-field v-model="experienceData.location.optional" placeholder="Apt., suite, building access code"></v-text-field>
                              </v-flex>
                              <v-flex xs12 sm12 md12 lg12 xl12>
                      ...
                      <div v-show="currentTab === 'Price'">
                        <v-container fluid>
                          <v-layout row wrap>
                            ...
                            <v-flex xs12 sm12 md4 lg4 xl4>
                              <v-text-field prefix="₱" type="number" v-model="experienceData.price"></v-text-field>
                            </v-flex>
                          </v-layout>
                        </v-container>
                      </div>
    ...
    setTimeArray () {
      for (let x = 1; x <= 12; x++) {
          if (x === 12) {
            this.timeArray.push(`${x}:00 PM`)
            this.timeArray.push(`${x}:30 PM`)
          } else {
            this.timeArray.push(`${x}:00 AM`)
            this.timeArray.push(`${x}:30 AM`)
          }
        }
        for (let x = 1; x <= 12; x++) {
          if (x === 12) {
        ...
        this.timeSelectionFrom = this.timeArray
        this.timeSelectionTo = this.timeArray
    },
    addTime () {
      ...
      if (timepickerFrom < timepickerTo) {
        ...
        this.timepickerFrom = null
        this.timepickerTo = null
        this.timeSelectionFrom.length = 0
        this.timeSelectionTo.length = 0
        this.setTimeArray()
      }
    }
  },
  watch: {
    ...
    timepickerFrom (val) {
      if (!val) return false
      const timepickerFrom = moment(this.timepickerFrom, 'h:mm A')
      this.timeSelectionTo = this.timeArray.filter((time) => {
    ...
    timepickerTo (val) {
      if (!val) return false
      const timepickerTo = moment(this.timepickerTo, 'h:mm A')
      this.timeSelectionFrom = this.timeArray.filter((time) => {
